...
special_pre_pep8_arg_name = pp.one_of(
    special_changes_arg_names, as_keyword=True
) + pp.FollowedBy("=")
special_pre_pep8_arg_name.set_parse_action(lambda t: special_changes_arg_names[t[0]])

pep8_converter = (
    special_pre_pep8_arg_name
    | pre_pep8_method_name
...
if __name__ == "__main__":
    ...
    argparser.add_argument(
        ...
        "-v",
        action="store_true",
        help="Show unified diff for each source file",
    )
    argparser.add_argument(
        "-vv",
        action="store_true",
        dest="verbose2",
        help="Show unified diff for each source file, plus names of scanned files with no changes",
    )
    argparser.add_argument(
        "--update", "-u", action="store_true", help="Update source files in-place"
    )
    argparser.add_argument(
        "--encoding",
        type=str,
        default="utf-8",
        help="Encoding of source files (default: utf-8)",
    )
    argparser.add_argument(
        "--exit-zero-even-if-changed",
        "-exit0",
        action="store_true",
        help="Exit with status code 0 even if changes were made",
    )
    argparser.add_argument(
        "source_filename",
        nargs="+",
        help="Source filenames or filename patterns of Python files to be converted",
    )
                ...
                if modified_contents != original_contents:
                    if args.update:
                        Path(filename).write_text(
                            modified_contents, encoding=args.encoding
                        )
