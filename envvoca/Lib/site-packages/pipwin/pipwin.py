    ...
    for link in links:
        if link.get("onclick") is not None:
            ...
            if regex_result is None:
                logger.info("Skip %s (wrong link format)" % unicode(link.string))
                continue
            ...
            if len(details) != 5:
                logger.info("Skip %s (wrong name format)" % unicode(link.string))
                continue
            else:
                logger.debug("Add %s" % unicode(link.string))
            ...
            if link.string:
                logger.debug("Skip %s (missing link)" % unicode(link.string))
    return data
