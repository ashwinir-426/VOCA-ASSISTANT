...
def append_arguments(code_obj, new_locals):
    ...
    name_translations = dict((co_names.index(name), i) for i, name in enumerate(names))
    ...
    varnames = co_varnames[:co_argcount] + new_locals + co_varnames[co_argcount:]
    ...
    for inst in instructions(code_obj):
        ...
        if inst.opcode == LOAD_GLOBAL:
            ...
            if idx in names_to_varnames:
                ...
                raise (
                    ValueError(
                        "a name was lost in translation last instruction %s" % str(inst)
                    )
                )
        # If it accesses co_varnames or co_names then update its argument.
            ...
            if (
                inst.argval not in code_obj.co_varnames[: code_obj.co_argcount]
            ):  # we do not need to remap existing arguments, they are not shifted by new ones.
                arg = inst.arg + len(new_locals)
        modified.extend(write_instruction(op, arg))
    if six.PY2:
        code = "".join(modified)
        args = (
            co_argcount + new_locals_len,
