...
class PyJs(object):
    ...
    def get(self, prop):  # external use!
        ...
        if self.Class == "Undefined" or self.Class == "Null":
            raise MakeError(
                "TypeError",
                "Undefined and null dont have properties (tried getting property %s)"
                % repr(prop),
            )
    ...
    def put(self, prop, val, op=None):  # external use!
        ...
        * / % + - << >> & ^ |"""
        if self.Class == "Undefined" or self.Class == "Null":
            raise MakeError(
                "TypeError",
                "Undefined and null don't have properties (tried setting property %s)"
                % repr(prop),
            )
        if not isinstance(prop, basestring):
            prop = prop.to_string().value
    ...
    def define_own_property(
        self, prop, desc
    ):  # Internal use only. External through Object
        ...
        if is_generic_descriptor(desc):
            ...
            if not configurable:
                if not current["writable"] and desc.get("writable"):
                    return False
            if (
                not current["writable"]
                and "value" in desc
                and current["value"] != desc["value"]
            ):
                return False
    ...
    def to_boolean(self):
        ...
        if typ == "Boolean":  # no need to convert
            ...
            return Js(
                bool(self.value and self.value == self.value)
            )  # test for nan (nan -> flase)
    ...
    def to_object(self):
        ...
        if typ == "Null" or typ == "Undefined":
            raise MakeError(
                "TypeError", "undefined or null can't be converted to object"
            )
    ...
    def __getitem__(self, item):
        return self.get(
            str(item) if not isinstance(item, PyJs) else item.to_string().value
        )
    ...
    def abstract_equality_comparison(self, other):
        ...
        elif (tx == "Undefined" and ty == "Null") or (
            ty == "Undefined" and tx == "Null"
        ):
            return true
    ...
    def contains(self, other):
        if not self.is_object():
            raise MakeError(
                "TypeError", "You can't use 'in' operator to search in non-objects"
            )
...
class JsObjectWrapper(object):
    ...
    def __iter__(self):
        if self._obj.Class in [
            ...
            raise MakeError(
                "TypeError", "%s is not iterable in Python" % self._obj.Class
            )

    def __repr__(self):
        if self._obj.is_primitive() or self._obj.is_callable():
            return repr(self._obj)
        elif self._obj.Class in (
            "Array",
            "Int8Array",
            "Uint8Array",
            "Uint8ClampedArray",
...
class PyJsFunction(PyJs):
    ...
    def __init__(self, func, prototype=None, extensible=True, source=None):
        ...
        self.func_name = (
            func.__name__ if not func.__name__.startswith("PyJs_anonymous") else ""
        )
        self.extensible = extensible
        self.prototype = prototype
        self.own = {}
    ...
    def call(self, this, args=()):
        ...
        arguments = PyJsArguments(
            args, self
        )  # tuple will be converted to arguments object.
...
class PyJsRegExp(PyJs):
    ...
    def __init__(self, regexp, prototype=None):
        ...
        try:
            ...
            raise MakeError(
                "SyntaxError",
                "Invalid RegExp pattern: %s -> %s" % (repr(self.value), repr(comp)),
            )
...
def define_error_type(name):
    ...
    def TypeError(message):
        return PyJsError(
            None if message.is_undefined() else message, TypeErrorPrototype
        )
...
setter = __proto__
ObjectPrototype.define_own_property(
    "__proto__",
    {"set": setter, "get": getter, "enumerable": False, "configurable": True},
)
...
fill_prototype(
    Uint8ClampedArrayPrototype, jstypedarray.TypedArrayPrototype, default_attrs
)
# Int16Array proto
...
def RegExp(pattern, flags):
    ...
    if len(set(flags)) != len(flags):
        raise MakeError(
            "SyntaxError", 'Invalid flags supplied to RegExp constructor "%s"' % flags
        )
