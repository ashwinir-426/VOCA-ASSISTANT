...
class ObjectMethods:
    ...
    def getOwnPropertyDescriptor(obj, prop):
        ...
        )  # will return undefined if we dont have this prop
    ...
    def defineProperties(obj, properties):
        ...
        for name in props:
            ...
            if not obj.define_own_property(name.value, desc):
                raise MakeError(
                    "TypeError", "Failed to define own property: %s" % name.value
                )
        return obj
...
def ToPropertyDescriptor(obj):  # page 38 (50 absolute)
    ...
    if obj.has_property("get"):
        ...
        if not (cand.is_undefined() or cand.is_callable()):
            raise MakeError(
                "TypeError", "Invalid getter (it has to be a function or undefined)"
            )
        desc["get"] = cand
    if obj.has_property("set"):
        ...
        if not (cand.is_undefined() or cand.is_callable()):
            raise MakeError(
                "TypeError", "Invalid setter (it has to be a function or undefined)"
            )
        desc["set"] = cand
