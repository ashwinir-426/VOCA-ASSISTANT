...
def parse(string):
    return PyJsDate(
        TimeClip(parse_date(string.to_string().value)), prototype=DatePrototype
    )


Date.define_own_property(
    "now",
    {"value": Js(now), "enumerable": False, "writable": True, "configurable": True},
)

Date.define_own_property(
    "parse",
    {"value": parse, "enumerable": False, "writable": True, "configurable": True},
)

Date.define_own_property(
    "UTC", {"value": UTC, "enumerable": False, "writable": True, "configurable": True}
)
...
def parse_date(py_string):  # todo support all date string formats
    ...
    z_formats = ("Z",) + ("",)
    supported_formats = [
        d + t + z for d in date_formats for t in hour_formats for z in z_formats
    ]
...
class DateProto:
    ...
    def setMilliseconds(ms):
        ...
        tim = MakeTime(Js(HourFromTime(t)), Js(MinFromTime(t)), Js(SecFromTime(t)), ms)
    ...
    def setMonth(month, date=None):
        ...
        if not date is None:
            ...
            dt = date.to_number()
        newDate = MakeDate(MakeDay(Js(YearFromTime(t)), m, dt), TimeWithinDay(t))
        u = TimeClip(LocalToUTC(newDate))
        this.value = u
        return u
    ...
    def toISOString():
        ...
        return ISO_FORMAT % (
            unicode(year) if 0 <= year <= 9999 else pad(year, 6, True),
            month,
            day,
            hour,
            minute,
...
DatePrototype.define_own_property(
    "constructor",
    {"value": Date, "enumerable": False, "writable": True, "configurable": True},
)
