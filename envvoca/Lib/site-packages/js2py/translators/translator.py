...
DEFAULT_HEADER = """from js2py.pyjs import *
# setting scope
...
def translate_js(
    js, HEADER=DEFAULT_HEADER, use_compilation_plan=False, parse_fn=pyjsparser_parse_fn
):
...
class match_unumerator(object):
    ...
    def __repr__(self):
        return "\n".join(
            self.placeholder_mask % counter + "=" + match
...
def get_compilation_plan(js):
    ...
    # using another re:
    compilation_plan = compilation_plan.replace("__PyJsNUM_", '"__PyJsNUM_').replace(
        "_PyJsNUM__", '_PyJsNUM__"'
    )
    compilation_plan = compilation_plan.replace("__PyJsSTR_", '"__PyJsSTR_').replace(
        "_PyJsSTR__", '_PyJsSTR__"'
    )
