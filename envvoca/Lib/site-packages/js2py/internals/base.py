...
class PyJs(object):
    ...
    def can_put(self, prop):  # to check
        ...
        if desc:  # if we have this property
            if is_accessor_descriptor(desc):
                return is_callable(desc["set"])  # Check if setter method is defined
            else:  # data desc
        ...
        for prop, kind in props:
            if prop in self.own:  # just check... probably will not happen very often.
                if is_data_descriptor(self.own[prop]):
                    ...
                    if (
                        kind == "i"
                        or (kind == "g" and "get" in self.own[prop])
                        or (kind == "s" and "set" in self.own[prop])
                    ):
                        raise MakeError(
                            "SyntaxError",
            ...
            if kind == "i":  # init
                ...
                self.define_own_property(
                    prop,
                    {"get": vals[i], "enumerable": True, "configurable": True},
                    False,
                )
            elif kind == "s":  # get
                self.define_own_property(
                ...
                raise RuntimeError(
                    "Invalid property kind - %s. Expected one of i, g, s." % repr(kind)
                )
            i += 1
...
class PyJsDate(PyJs):
    ...
    def local_strftime(self, pattern):
        ...
        try:
            ...
            raise MakeError(
                "TypeError", "unsupported date range. Will fix in future versions"
            )
        try:
...
class Scope(PyJs):
    ...
    def __init__(self, scope, space, parent=None):
        ...
        if type(scope) is not dict:
            assert (
                parent is not None
            ), "You initialised the WITH_SCOPE without a parent scope."
            self.own = scope
    ...
    def get(self, var, throw=False):
        if self.prototype is not None:
            if self.is_with_scope:
                cand = None if not self.own.has_property(var) else self.own.get(var)
            else:
                # fast local scope
...
class PyJsFunction(PyJs):
    ...
    def __init__(
        ...
        if (
            type(self.code) == int
        ):  # just a label pointing to the beginning of the code.
    ...
    def _generate_my_context(self, this, args):
        ...
        if not self.is_declaration and self.name and self.name not in my_ctx.own:
            my_ctx.own[self.name] = (
                self  # this should be immutable binding but come on!
            )
        return my_ctx
