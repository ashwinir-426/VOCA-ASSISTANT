...
def set_protected(obj, name, prop):
    obj.define_own_property(
        unicode(name),
        {"value": prop, "writable": False, "enumerable": False, "configurable": False},
        True,
    )
...
def fill_space(space, byte_generator):
    ...
    space.FunctionPrototype = (
        function_proto  # this will fill the prototypes of the methods!
    )
    ...
    json.put("stringify", easy_func(jsjson.stringify, space))
    json.put("parse", easy_func(jsjson.parse, space))
    ...
    # Error
    error_proto = space.NewError("Error", "")
    error_proto.prototype = object_proto
    error_proto.put("name", "Error")
    ...
    for err_type_name in (
        ...
        extra_err = space.NewError("Error", "")
        extra_err.put("name", err_type_name)
        setattr(space, err_type_name + "Prototype", extra_err)
        error_constructors[err_type_name] = construct_constructor(err_type_name)

    assert space.TypeErrorPrototype is not None

    # RegExp
    regexp_proto = space.NewRegExp("(?:)", "")
    ...
    array.put("isArray", easy_func(jsarray.isArray, space))
