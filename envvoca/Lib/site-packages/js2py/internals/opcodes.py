    ...
    def eval(self, ctx):
        ...
        if typ in PRIMITIVES:
            ...
            if typ == NULL_TYPE:
                ...
                raise MakeError(
                    "TypeError", "Cannot set property '%s' of undefined" % prop
                )
            # just ignore...
        else:
...
class STORE_MEMBER_DOT(OP_CODE):
    ...
    def eval(self, ctx):
        ...
        if typ in PRIMITIVES:
            if typ == NULL_TYPE:
                raise MakeError(
                    "TypeError", "Cannot set property '%s' of null" % self.prop
                )
            elif typ == UNDEFINED_TYPE:
                raise MakeError(
                    "TypeError", "Cannot set property '%s' of undefined" % self.prop
                )
            # just ignore...
        else:
...
class STORE_MEMBER_OP(OP_CODE):
    ...
    def eval(self, ctx):
        ...
        if typ in PRIMITIVES:
            if typ is NULL_TYPE:
                raise MakeError(
                    "TypeError", "Cannot set property '%s' of null" % to_string(name)
                )
            ...
            ctx.stack.append(
                BINARY_OPERATIONS[self.op](get_member(left, name, ctx.space), value)
            )
            return
        else:
            ctx.stack.append(
...
class STORE_MEMBER_DOT_OP(OP_CODE):
    ...
    def eval(self, ctx):
        ...
        if typ in PRIMITIVES:
            ...
            ctx.stack.append(
                BINARY_OPERATIONS[self.op](
                    get_member_dot(left, self.prop, ctx.space), value
                )
            )
            return
        else:
...
class TRY_CATCH_FINALLY(OP_CODE):
    ...
    def eval(self, ctx):
        ...
        if errors and self.catch_variable is not None:
            ...
            catch_context = Scope(
                {self.catch_variable: try_status[2].get_thrown_value(ctx.space)},
                ctx.space,
                ctx,
            )
...
class WITH(OP_CODE):
    ...
    def eval(self, ctx):
        ...
        with_context = Scope(
            obj, ctx.space, ctx
        )  # todo actually use the obj to modify the ctx
