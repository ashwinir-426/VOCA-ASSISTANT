...
class ArrayPrototype:
        ...
        for i in xrange(arr_len):
            ...
            if is_undefined(element) or is_null(element):
                ...
                if not is_callable(str_func):
                    raise MakeError(
                        "TypeError",
                        "toLocaleString method of item at index %d is not callable" % i,
                    )
    ...
    def join(this, args):
        ...
        for e in xrange(arr_len):
            ...
            if elem in ARR_STACK:
                ...
                s = to_string(elem)
            elems.append(s if not (is_undefined(elem) or is_null(elem)) else "")
    ...
    def reverse(this, args):
        ...
        has_props = [
            array.has_property(unicode(e)) for e in xrange(js_arr_length(array))
        ]
        vals.reverse()
        has_props.reverse()
    ...
    def slice(this, args):  # todo check
        ...
        k = (
            max((arr_len + relative_start), 0)
            if relative_start < 0
            else min(relative_start, arr_len)
        )
        relative_end = arr_len if is_undefined(end) else to_int(end)
        final = (
            max((arr_len + relative_end), 0)
            if relative_end < 0
            else min(relative_end, arr_len)
        )
        res = []
        n = 0
        while k < final:
