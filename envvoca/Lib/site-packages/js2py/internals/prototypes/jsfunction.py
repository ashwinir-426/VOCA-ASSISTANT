...
class FunctionPrototype:
    def toString(this, args):
        ...
        args = ", ".join(map(unicode, this.params))
        return "function %s(%s) { [native code] }" % (
            this.name if this.name else "",
            args,
        )
    ...
    def bind(this, args):
        ...
        js_bound = args.space.NewFunction(bound, this.ctx, (), "", False, ())
        js_bound.put("length", float(max(len(this.params) - len(bound_args), 0.0)))
        js_bound.put("name", "boundFunc")
        return js_bound
