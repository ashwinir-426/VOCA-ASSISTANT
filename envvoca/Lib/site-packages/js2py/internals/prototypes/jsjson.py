...
def parse(this, args):
    ...
    try:
        ...
        raise MakeError(
            "SyntaxError", "JSON.parse could not parse JSON string - Invalid syntax"
        )
...
def stringify(this, args):
    ...
    return Str(
        "",
        args.space.ConstructObject({"": value}),
        replacer_function,
        property_list,
        gap,
        stack,
...
def jo(value, stack, gap, property_list, replacer_function, space):
    ...
    for p in k:
        ...
        if not is_undefined(str_p):
            member = (
                json.dumps(p) + ":" + (" " if gap else "") + str_p
...
def ja(value, stack, gap, property_list, replacer_function, space):
    ...
    for index in xrange(length):
        index = unicode(index)
        str_index = Str(
            index, value, replacer_function, property_list, gap, stack, space
        )
