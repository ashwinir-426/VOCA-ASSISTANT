...
def parse(text):
    ...
    try:
        ...
        raise this.MakeError(
            "SyntaxError", "Could not parse JSON string - Invalid syntax"
        )
...
def jo(value, stack, gap, property_list, replacer_function, space):
    ...
    for p in k:
        ...
        if not str_p.is_undefined():
            member = (
                json.dumps(p) + ":" + (" " if gap else "") + str_p.value
            )  # todo not sure here - what space character?
...
def ja(value, stack, gap, property_list, replacer_function, space):
    ...
    for index in xrange(length):
        index = str(index)
        str_index = value.Js(
            Str(index, value, replacer_function, property_list, gap, stack, space)
        )
...
JSON.define_own_property(
    "parse",
    {"value": Js(parse), "enumerable": False, "writable": True, "configurable": True},
)
