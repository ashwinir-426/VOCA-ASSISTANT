...
class Element:
    ...
    def queryTaggedValue(self, tag, default=None):
        """Returns the value associated with 'tag'."""
        return (
            self.__tagged_values.get(
                tag,
                default,
            )
            if self.__tagged_values
            else default
        )
...
class Specification(SpecificationBase):
    ...
    def changed(self, originally_changed):
        ...
        self.__iro__ = tuple(
            [ancestor for ancestor in ancestors if isinstance(ancestor, InterfaceClass)]
        )
    ...
    def __new__(
        ...
        if needs_custom_class:
            ...
            cls = type(cls)(  # pylint:disable=self-cls-assignment
                name + "<WithCustomMethods>", cls_bases, needs_custom_class
            )
...
class Attribute(Element):
    ...
    def __repr__(self):
        return "<{}.{} object at 0x{:x} {}>".format(
            type(self).__module__, type(self).__name__, id(self), self
        )
