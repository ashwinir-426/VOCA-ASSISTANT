...
class ThreadTests(unittest.TestCase):
    ...
    def test_various_ops(self):
        ...
        for i in range(NUMTASKS):
            ...
            self.assertTrue(re.match(initial_regex, repr(t)), repr(t))
            t.start()
    ...
    def test_limbo_cleanup(self):
        ...
        if hasattr(threading, "_start_new_thread"):
            ...
            patcher = Patch.object(
                threading, "_start_joinable_thread", new=fail_new_thread
            )
...
%s
    ...
    @greentest.skipIf(
        not (hasattr(sys, "getcheckinterval")), "Needs sys.getcheckinterval"
    )
    ...
    if not hasattr(sys, "pypy_version_info"):
        ...
        def test_no_refcycle_through_target(self):
            class RunSelfFunction(object):
                def __init__(self, should_raise):
                    ...
                    self.thread = threading.Thread(
                        target=self._run, args=(self,), kwargs={"_yet_another": self}
                    )
                    self.thread.start()
            ...
            self.assertIsNone(
                weak_cyclic_object(),
                msg=(
                    "%d references still around" % sys.getrefcount(weak_cyclic_object())
                ),
            )
...
%s
        ...
        p = subprocess.Popen(
            [sys.executable, "-W", "ignore", "-c", script], stdout=subprocess.PIPE
        )
        rc = p.wait(60)
