...
class TestFileObjectBlock(CleanupMixin, greentest.TestCase):
    ...
    def __check_native_matches(
        self, byte_data, open_mode, meth="read", open_path=True, **open_kwargs
    ):
    ...
    def test_text_encoding(self):
        gevent_data = self.__check_native_matches(
            "\N{SNOWMAN}".encode("utf-8"), "r+", buffering=5, encoding="utf-8"
        )
        self.assertIsInstance(gevent_data, str)
...
class TestFileObjectThread(
    ConcurrentFileObjectMixin, TestFileObjectBlock  # pylint:disable=too-many-ancestors
):
...
class TestOpenDescriptor(CleanupMixin, greentest.TestCase):
    ...
    def test_atomicwrite_fd(self):
        ...
        desc = self._makeOne(
            fileno, "wb", buffering=0, closefd=False, atomic_write=True
        )
...
def pop():
    for regex, kind, kwargs in TestOpenDescriptor.CASES:
        setattr(
            TestOpenDescriptor,
            "test_" + regex.replace(" ", "_"),
            lambda self, _re=regex, _kind=kind, _kw=kwargs: self._check(
                _re, _kind, 1, **_kw
            ),
        )
