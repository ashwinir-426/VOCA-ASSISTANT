...
PY313LT5 = ConstantCondition(
    sys.version_info[:2] == (3, 13) and sys.version_info[2] < 5
)
...
class Definitions(metaclass=DefinitionsMeta):
    ...
    test_subprocess = (
        ...
        .ignored(
            "Tests don't even start before the process times out.", when=PYPY3 & WIN
        )
    )
    ...
    test__doctests = Ignored(
        "Sometimes times out during/after gevent._config.Config", when=CI & OSX
    )
