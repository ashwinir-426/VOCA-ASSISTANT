    ...
    def _assert_sock_open(self, sock):
        ...
        for x in open_files["data"]:
            if getattr(x, "laddr", None) == sockname:
                assert (
                    x.status
                    in (psutil.CONN_LISTEN, psutil.CONN_ESTABLISHED)
                    + self.extra_allowed_open_states
                ), x.status
                return
...
class TestSSL(Test):
    ...
    def _make_ssl_connect_task(self, connector, port):
        ...
        t = threading.Thread(
            target=self._ssl_connect_task, args=(connector, port, accepted_event)
        )
        t.daemon = True
    ...
    def test_server_simple(self):
        with Closing() as closer:
            ...
            client_socket = closer(
                self._wrap_socket(
                    client_socket, keyfile=CERTFILE, certfile=CERTFILE, server_side=True
                )
            )
            fileno = client_socket.fileno()
            self.assert_open(client_socket, fileno)
