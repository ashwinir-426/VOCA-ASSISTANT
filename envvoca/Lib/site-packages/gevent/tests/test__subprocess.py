...
class TestPopen(greentest.TestCase):
    ...
    def test_communicate_undecodable(self):
        ...
        with subprocess.Popen(
            [
                ...
                "ignore",
                "-c",
                "import os, sys; " r'os.write(sys.stdout.fileno(), b"\xff")',
            ],
            stdin=subprocess.PIPE,
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
    ...
    def test_check_output_keyword_error(self):
        ...
        ) as exc:  # pylint:disable=no-member
    ...
    def test_subprocess_in_native_thread(self):
        ...
        self.assertEqual(
            ex[0].args[0], "child watchers are only available on the default loop"
        )

    @greentest.skipOnLibuvOnPyPyOnWin("hangs")
    def __test_no_output(self, kwargs, kind):
        with subprocess.Popen(
            [sys.executable, "-c", "pass"], stdout=subprocess.PIPE, **kwargs
        ) as proc:
    ...
    @greentest.skipOnLibuvOnCIOnPyPy(
        "Sometimes segfaults; " "https://travis-ci.org/gevent/gevent/jobs/327357682"
    )
...
class RunFuncTestCase(greentest.TestCase):
    ...
    def test_capture_stderr(self):
        cp = self.run_python(
            "import sys; sys.stderr.write('BDFL')", stderr=subprocess.PIPE
        )
