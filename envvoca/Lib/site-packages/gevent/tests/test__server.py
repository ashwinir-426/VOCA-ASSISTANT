...
class TestCase(greentest.TestCase):
    ...
    @contextmanager
    def makefile(
        self, timeout=_DEFAULT_SOCKET_TIMEOUT, bufsize=1, include_raw_socket=False
    ):
    ...
    def assertConnectionRefused(self, in_proc_server=True):
        ...
        self.assertIn(
            ...
            + self.LOCAL_CONN_REFUSED_ERRORS,
            (ex, ex.args),
        )
    ...
    def assertNotAccepted(self):
        ...
        self.assertTrue(
            result.startswith(b"HTTP/1.0 500 Internal Server Error"), repr(result)
        )
    ...
    def ServerSubClass(self, *args, **kwargs):
        return self.Settings.ServerSubClass(
            *args, **self.fill_default_server_args(kwargs)
        )
...
class TestDefaultSpawn(TestCase):
    ...
    @greentest.skipIf(
        greentest.EXPECT_POOR_TIMER_RESOLUTION, "Sometimes spuriously fails"
    )
    def test_server_closes_client_sockets(self):
        self.server = self.ServerClass(
            (greentest.DEFAULT_BIND_ADDR, 0), lambda *args: []
        )
        self.server.start()
        sleep_to_clear_old_sockets()
        with self.makefile() as conn:
    ...
    def test_server_repr_when_handle_is_instancemethod(self):
        ...
        self.assertIn(
            "test_server_repr_when_handle_is_instancemethod", repr(self.server)
        )
...
class TestPoolSpawn(TestDefaultSpawn):
    ...
    @greentest.skipIf(
        greentest.EXPECT_POOR_TIMER_RESOLUTION,
...
class TestSSLGetCertificate(TestCase):
    ...
    def _create_server(self):  # pylint:disable=arguments-differ
        return self.ServerSubClass(
            (greentest.DEFAULT_BIND_ADDR, 0),
