...
class AbstractLinkable(object):
    ...
    def _handle_unswitched_notifications(self, unswitched):
        ...
        while unswitched:
            ...
            if getattr(link, "__name__", None) == "switch" and isinstance(
                getattr(link, "__self__", None), greenlet
            ):
                glet = link.__self__
                parent = glet.parent
    ...
    def __print_unswitched_warning(self, link, printed_tb):
        print(
            "gevent: error: Unable to switch to greenlet",
            link,
            "from",
            self,
            "; crossing thread boundaries is not allowed.",
            file=sys.stderr,
        )
        ...
        if not printed_tb:
            ...
            print(
                ...
                "and is a bug in the calling code.",
                file=sys.stderr,
            )
    ...
    def _wait(self, timeout=None):
        ...
        if self.ready():  # *might* release, if overridden in Python.
            result = self._wait_return_value(
                False, False
            ...
            if self._notifier:
                ...
                self.__wait_to_be_notified(
                    False
                )  # Use self._notifier.args[0] instead of self.rawlink
