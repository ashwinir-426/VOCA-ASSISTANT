    ...
    def __init__(self, listener, handle=None, spawn="default"):
        ...
        try:
            ...
            if self.max_accept < 1:
                raise ValueError(
                    "max_accept must be positive int: %r" % (self.max_accept,)
                )
        except:
            self.close()
            raise
    ...
    def set_listener(self, listener):
        if hasattr(listener, "accept"):
            if hasattr(listener, "do_handshake"):
                raise TypeError(
                    "Expected a regular socket, not SSLSocket: %r" % (listener,)
                )
...
def _parse_address(address):
    ...
    if (isinstance(address, string_types) and ":" not in address) or isinstance(
        address, integer_types
    ):  # noqa (pep8 E129)
        # Just a port
