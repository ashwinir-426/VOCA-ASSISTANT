...
class ResultCollector(object):
    ...
    def longest_running_tests(self):
        ...
        """
        return sorted(
            self._all_results, key=operator.attrgetter("run_duration"), reverse=True
        )
...
class Runner(object):
    ...
    def __call__(self):
        util.log(
            "Running tests in parallel with concurrency %s %s."
            % (
                self._worker_count,
                util._colorize(
                    "number", "(concurrency available: %d)" % AVAIL_NWORKERS
                ),
            ),
        )
...
class Discovery(object):
    ...
    class Discovered(object):
        def __init__(
            self, package, configured_test_options, ignore, config, allow_combine
        ):
        ...
        def __makes_simple_monkey_patch(
            contents,
            _patch_present=re.compile(rb"[^#].*patch_all\(\)"),
            _patch_indented=re.compile(rb"    .*patch_all\(\)"),
        ):
            return (
        ...
        def __calls_unittest_main_toplevel(
            contents,
            _greentest_main=re.compile(rb"    greentest.main\(\)"),
            _unittest_main=re.compile(rb"    unittest.main\(\)"),
            _import_main=re.compile(rb"from gevent.testing import.*main"),
            _main=re.compile(rb"    main\(\)"),
        ):
        ...
        @staticmethod
        def __makes_no_monkey_patch(
            contents, _patch_present=re.compile(rb"[^#].*patch_\w*\(")
        ):
        ...
        def __add_test(self, qualified_name, filename, contents):
            if b"TESTRUNNER" in contents:  # test__monkey_patching.py
                ...
                self.std_monkey_patch_files.append(
                    qualified_name if self.package else filename
                )
            elif self.__can_nonmonkey_combine(filename, contents):
                self.no_monkey_patch_files.append(
                    qualified_name if self.package else filename
                )
            else:
        ...
        def visit_file(self, filename):
            ...
            if filename.startswith("gevent.tests"):
                ...
                qualified_name = (
                    self.package + "." + module_name if self.package else module_name
                )
...
def _show_longest_running(result_collector, how_many=5):
    ...
    length_of_longest_formatted_decimal = len(
        "%.1f" % longest_running_tests[0].run_duration
    )
...
def report(
    result_collector,  # type: ResultCollector
    exit=True,
    took=None,
    configured_failing_tests=(),
):
    ...
    util.log(
        ...
        total_cases,
        (
            util._colorize("skipped", " (skipped=%d)" % total_skipped)
            if total_skipped
            else ""
        ),
        total,
...
def _setup_environ(debug=False):
    ...
    interesting_envs = {
        k: os.environ[k] for k in os.environ if k.startswith(("PYTHON", "GEVENT"))
    }
...
def main():
    ...
    parser.add_argument(
        "--discover", action="store_true", help="Only print the tests found."
    )
    parser.add_argument(
        "--config",
        default="known_failures.py",
    ...
    if options.coverage or os.environ.get("GEVENTTEST_COVERAGE"):
        if PYPY and RUNNING_ON_CI:
            ...
            coverage = True
            cov_config = os.environ["COVERAGE_PROCESS_START"] = os.path.abspath(
                ".coveragerc"
            )
            if PYPY:
                cov_config = os.environ["COVERAGE_PROCESS_START"] = os.path.abspath(
    ...
    if options.discover:
        for cmd, options in tests:
            print(
                util.getname(cmd, env=options.get("env"), setenv=options.get("setenv"))
            )
        ...
        if options.package:
            ...
            os.environ["PYTHONPATH"] = (
                os.environ.get("PYTHONPATH", "") + os.pathsep + package_dir
            )
