...
if PY313 and WIN:
    __imports__.extend(["STARTF_FORCEONFEEDBACK", "STARTF_FORCEOFFFEEDBACK"])
...
actually_imported = copy_globals(
    __subprocess__, globals(), only_names=__imports__, ignore_missing_names=True
)
    ...
    def __init__(
        ...
        if p2cwrite != -1:
            if text_mode:
                ...
                self.stdin = FileObject(
                    p2cwrite, "w", bufsize, encoding=self.encoding, errors=self.errors
                )
            else:
        ...
        if c2pread != -1:
            if universal_newlines or text_mode:
                self.stdout = FileObject(
                    c2pread, "r", bufsize, encoding=self.encoding, errors=self.errors
                )
        ...
        if errread != -1:
            if universal_newlines or text_mode:
                self.stderr = FileObject(
                    errread, "r", bufsize, encoding=encoding, errors=errors
                )
            else:
    ...
    def __handle_uids(self, user, group, extra_groups):
        ...
        if extra_groups is not None:
            ...
            for gid_check in gids:
                if gid_check < 0:
                    raise ValueError(
                        "Group ID cannot be negative, got %s" % (gid_check,)
                    )
    ...
    if mswindows:
        ...
        def _find_w9xpopen(self):
            ...
            w9xpopen = os.path.join(
                os.path.dirname(GetModuleFileName(0)), "w9xpopen.exe"
            )
            if not os.path.exists(w9xpopen):
                # Eeek - file-not-found - possibly an embedding
                # situation - see if we can locate it in sys.exec_prefix
                w9xpopen = os.path.join(
                    os.path.dirname(sys.exec_prefix), "w9xpopen.exe"
                )
                if not os.path.exists(w9xpopen):
                    raise RuntimeError(
                        "Cannot locate w9xpopen.exe, which is "
        ...
        def _execute_child(
            ...
            try:
                try:
                    ...
                    gc.disable()
                    try:
                        self.pid = fork_and_watch(
                            self._on_child, self._loop, True, fork
                        )
                    except:
                        if gc_was_enabled:
                            gc.enable()
                        ...
                        try:
                            ...
                            try:
                                if env is None:
                                    ...
                                    env = {
                                        os.fsdecode(k) if isinstance(k, bytes) else k: (
                                            os.fsdecode(v)
...
class CompletedProcess(object):
    ...
    def check_returncode(self):
        ...
        if self.returncode:
            # pylint:disable=undefined-variable
            raise _with_stdout_stderr(
                CalledProcessError(self.returncode, self.args, self.stdout), self.stderr
            )
...
def run(*popenargs, **kwargs):
    ...
    with Popen(*popenargs, **kwargs) as process:
        try:
            ...
            raise _with_stdout_stderr(
                TimeoutExpired(process.args, timeout, output=stdout), stderr
            )
        except:
            process.kill()
            process.wait()
        ...
        if check and retcode:
            # pylint:disable=undefined-variable
            raise _with_stdout_stderr(
                CalledProcessError(retcode, process.args, stdout), stderr
            )
