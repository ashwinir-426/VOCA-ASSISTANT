...
class PeriodicMonitoringThread(object):
    ...
    def __init__(self, hub):
        ...
        self._monitoring_functions = [
            _MonitorEntry(self.monitor_blocking, GEVENT_CONFIG.max_blocking_time)
        ]
    ...
    def monitoring_functions(self):
        ...
        )  # XXX: Events so we know when this changes.
    ...
    def monitor_blocking(self, hub):
        ...
        notify(
            EventLoopBlocked(
                active_greenlet, GEVENT_CONFIG.max_blocking_time, report, hub=hub
            )
        )
    ...
    def install_monitor_memory_usage(self):
        ...
        if not self.can_monitor_memory_usage():
            ...
            warnings.warn(
                "Unable to monitor memory usage. Install psutil.", MonitorWarning
            )
            return

        self.add_monitoring_function(
            self.monitor_memory_usage,
            max(GEVENT_CONFIG.memory_monitor_period, self.min_memory_monitor_period),
        )
    ...
    def monitor_memory_usage(self, _hub):
        ...
        if mem_usage > max_allowed:
            if mem_usage > self._memory_exceeded:
                # We're still growing
                event = MemoryUsageThresholdExceeded(mem_usage, max_allowed, rusage)
                notify(event)
