...
class BasePatcher:
    ...
    def __call__(self):
        ...
        gevent_thread_mod, thread_mod = _patch_module(
            "thread", _warnings=self.warnings, _notify_did_subscribers=False
        )
        ...
        if (
            self.orig_current_thread == self.threading_mod.main_thread()
        ...
        _notify_patch(
            events.GeventDidPatchModuleEvent("thread", gevent_thread_mod, thread_mod)
        )
        if self.gevent_threading_mod is not None:
            _notify_patch(
                events.GeventDidPatchModuleEvent(
                    "threading", self.gevent_threading_mod, self.threading_mod
                )
            )
    ...
    def patch_threading_event_logging_existing_locks(self):
        ...
        self.gevent_threading_mod, patched_mod = _patch_module(
            "threading", _warnings=self.warnings, _notify_did_subscribers=False
        )
    ...
    def patch_main_thread_cleanup(self):
        ...
        if oldid in self.threading_mod._active:
            self.threading_mod._active[main_thread.ident] = self.threading_mod._active[
                oldid
            ]
    ...
    def _make_existing_non_main_thread_join_func(
        thread, thread_greenlet, threading_mod
    ):
