...
def patch_socket(dns=True, aggressive=True):
    ...
    if dns:
        ...
        items = set(socket.__implements__) - set(
            socket.__dns__
...
def _find_module_refs(to, excluding_names=()):
    ...
    for r in gc.get_referrers(to):
        if isinstance(r, dict) and "__name__" in r:
            ...
            isinstance(r, type) and to in r.__bases__ and "gevent." not in r.__module__
        ):
    ...
    if may_need_warning:
        ...
        if direct_ref_modules or subclass_modules:
            ...
            if direct_ref_modules:
                direct_ref_mod_str = (
                    "Modules that had direct imports (NOT patched): %s. "
                    % (
                        [
                            "%s (%s)" % (name, fname)
...
def patch_select(aggressive=True):
    ...
    """
    _patch_module("select", _patch_kwargs={"aggressive": aggressive})
...
def patch_selectors(aggressive=True):
    ...
    _patch_module("selectors", _patch_kwargs={"aggressive": aggressive})
...
def _subscribe_signal_os(will_patch_all):
    if will_patch_all.will_patch_module(
        "signal"
    ) and not will_patch_all.will_patch_module("os"):
...
def patch_all(
    ...
    **kwargs
    ...
    try:
        _notify_patch(
            events.GeventWillPatchAllEvent(modules_to_patch, kwargs), _warnings
        )
    ...
    _notify_patch(
        events.GeventDidPatchBuiltinModulesEvent(modules_to_patch, kwargs), _warnings
    )
