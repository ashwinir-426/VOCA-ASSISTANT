...
_allocate_lock, _get_ident = monkey.get_original(
    ("_thread", "thread"), ("allocate_lock", "get_ident")
)
...
class _GILLock(object):
    ...
    def release(self):
        ...
        if current_tid != self._owned_thread_id:
            raise RuntimeError(
                "%s: Releasing lock not owned by you. You: 0x%x; Owner: 0x%x"
                % (
                    self,
                    current_tid,
                    self._owned_thread_id or 0,
                )
            )
...
class _AtomicSemaphoreMixin(object):
    ...
    def _notify_links(self, arrived_while_waiting):
        with self._lock_lock:
            return super(_AtomicSemaphoreMixin, self)._notify_links(
                arrived_while_waiting
            )
...
class RLock(object):
    ...
    def release(self):
        ...
        if self._owner is not getcurrent():
            raise RuntimeError(
                "cannot release un-acquired lock. Owner: %r Current: %r"
