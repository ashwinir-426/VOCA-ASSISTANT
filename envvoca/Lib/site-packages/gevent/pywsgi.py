...
class WSGIHandler(object):
        ...
        if len(words) == 3:
            ...
            if not self._check_http_version():
                raise _InvalidClientRequest(
                    "Invalid http version: %r" % (raw_requestline,)
                )
        elif len(words) == 2:
            self.command, self.path = words
            if self.command != "GET":
                raise _InvalidClientRequest(
                    "Expected GET method; Got command=%r; path=%r; raw=%r"
                    % (
                        self.command,
                        self.path,
                        raw_requestline,
    ...
    def start_response(self, status, headers, exc_info=None):
        ...
        if self.code in (304, 204):
            if (
                self.provided_content_length is not None
                and self.provided_content_length != "0"
            ):
                msg = (
...
class WSGIServer(StreamServer):
    ...
    base_env = {
        ...
        % (gevent.version_info[:2] + sys.version_info[:2]),
        "SCRIPT_NAME": "",
        "wsgi.version": (1, 0),
    ...
    def __init__(
        ...
        **ssl_args
        ...
        if "handle" in ssl_args:
            ...
            warnings.warn(
                "Passing 'handle' kwarg to WSGIServer is deprecated. "
