...
class HashFailedException(Exception):
    ...
    def __str__(self):
        return "HashFailedException({}, got {}, expected {})".format(
            self.filename, self.calculated_hash, self.needed_hash
        )
...
class SmartDL:
    ...
    def __init__(
        ...
        self.shared_var = multiprocessing.Value(
            c_int, 0
        )  # a ctypes var that counts the bytes already downloaded
        ...
        if os.path.exists(self.dest):
            self.logger.warning(
                'Destination "{}" already exists. Existing file will be removed.'.format(
                    self.dest
                )
            )
        if not os.path.exists(os.path.dirname(self.dest)):
        ...
        self.logger.info(
            "Creating a ThreadPool of {} thread(s).".format(self.threads_count)
        )
    ...
    def start(self, blocking=None):
        ...
        if not self.status == "ready":
            raise RuntimeError(
                "cannot start (current status is {})".format(self.status)
            )
        ...
        if self.mirrors:
            self.logger.info(
                "One URL and {} mirrors are loaded.".format(len(self.mirrors))
            )
        else:
        ...
        args = utils.calc_chunk_size(
            self.filesize, self.threads_count, self.minChunkFile
        )
        ...
        if len(args) > 1:
            ...
            self.logger.info(
                "Launching 1 thread (downloads {}).".format(
    ...
    def isSuccessful(self):
        ...
        while self.status != "finished":
            ...
            if n >= 15:
                raise RuntimeError(
                    "The download task must be finished in order to see if it's successful. (current status is {})".format(
                        self.status
                    )
                )
    ...
    def get_data(self, binary=False, bytes=-1):
        ...
        if self.status != "finished":
            raise RuntimeError(
                "The download task must be finished in order to read the data. (current status is %s)"
                % self.status
            )
...
def post_threadpool_actions(pool, args, expected_filesize, SmartDLObj):
    ...
    if SmartDLObj.verify_hash:
        ...
        if hash_ == SmartDLObj.hash_code:
            ...
            SmartDLObj.try_next_mirror(
                HashFailedException(
                    os.path.basename(dest_path), hash, SmartDLObj.hash_code
                )
            )
