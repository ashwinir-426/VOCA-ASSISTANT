...
def download(
    ...
    try:
        ...
        if e.code == 416:
            ...
            if retries > 0:
                logger.warning(
                    "Thread didn't got the file it was expecting. Retrying ({} times left)...".format(
                        retries - 1
                    )
                )
                time.sleep(5)
            ...
            if thread_shared_cmds:
                ...
                if "limit" in thread_shared_cmds:
                    ...
                    if time_passed > 0.1:  # we only observe the limit after 100ms
                        ...
                        if (
                            filesize_dl - limitspeed_filesize
                        ) / time_passed >= thread_shared_cmds["limit"]:
                            time_to_sleep = (
                                filesize_dl - limitspeed_filesize
                            ) / thread_shared_cmds["limit"]
                            logger.debug(
                                "Thread has downloaded {} in {}. Limit is {}/s. Slowing down...".format(
                                    utils.sizeof_human(
                                        filesize_dl - limitspeed_filesize
                                    ),
                                    utils.time_human(
                                        time_passed, fmt_short=True, show_ms=True
                                    ),
                                    utils.sizeof_human(thread_shared_cmds["limit"]),
                                )
                            )
