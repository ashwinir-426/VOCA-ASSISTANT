...
def _playsoundWin(sound, block=True):
    ...
    if any((c in sound for c in " \"'()")):
        ...
        fd, tempPath = mkstemp(
            prefix="PS", suffix=splitext(sound)[1]
        )  # Avoid generating files longer than 8.3 characters.
        logger.info(
        ...
        close(
            fd
        )  # mkstemp opens the file, but it must be closed before MCI can open it.
        try:
